...
const ctx = {
  ...
  publicUser,
  ...httpUtils,
  getStudentByUser: (pool, userId) => services.getStudentByUser(pool, sql, userId),
  confirmedPaid: (pool, applicationId) => services.confirmedPaid(pool, sql, applicationId),
  roomOccupancy: (pool, roomId) => services.roomOccupancy(pool, sql, roomId),
  getApplicationForAccess: (pool, applicationId, user) => services.getApplicationForAccess(pool, sql, applicationId, user)
};
